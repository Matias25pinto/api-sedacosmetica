...
const productosMasVendidosDelDia = async () => {
  try {
    ...
    const result =
      await sql.query`SELECT v.codigobarra, v.nombreproducto, p.codigo as "codigoInterno", SUM(v.cantidad) as "cantidad" 
                                        FROM VENTAS_SMARKET_VIEW v
					JOIN Producto p
					ON v.codigobarra = p.codigobarra
                                        WHERE v.fecha = ${condicion} 
										AND p.empresa = 4
                                        GROUP BY v.codigobarra, v.nombreproducto, p.codigo 
                                        ORDER BY 3 DESC
                                        OFFSET 0 ROWS
...
const nuevosProductos = async () => {
  try {
    await sql.connect(sqlConfig);
    const result =
      await sql.query`SELECT p.codigobarra, p.codigo as "codigoInterno", p.descripcion, empresa
                                               FROM Producto p
                                               WHERE empresa = 4
                                               ORDER BY id DESC
                                               OFFSET 0 ROWS 
...
const buscarProductos = async (termino, iddeposito, desde, hasta) => {
  try {
    await sql.connect(sqlConfig);
    const result =
      await sql.query`SELECT p.id, p.codigobarra,  p.codigo as "codigoInterno", p.descripcion, p.descripcionale, p.empresa, s.iddeposito, d.codigo, SUM(s.existencia) as "existencia", p.pcosto, p.pcostod, p.preven, p.preven2, p.preven3, p.preven4, p.preven5, p.preven6, p.preven7, p.preven8, p.preven9, p.preven10, p.preven11, p.preven12, p.preven13, p.preven14, p.preven15, p.preven16, p.preven17, p.preven18, p.preven19, p.preven20
   FROM Producto p
    ...
    GROUP BY p.id, p.codigobarra, p.codigo, p.descripcion, p.descripcionale, p.empresa, s.iddeposito, d.codigo, p.pcostod, p.pcosto, p.preven, p.preven2, p.preven3, p.preven4, p.preven5, p.preven6, p.preven7, p.preven8, p.preven9, p.preven10, p.preven11, p.preven12, p.preven13, p.preven14, p.preven15, p.preven16, p.preven17, p.preven18, p.preven19, p.preven20
    ORDER BY 1 DESC
    OFFSET ${desde} ROWS
...
const buscarProductosCB = async (
  ...
  try {
    await sql.connect(sqlConfig);
    const result =
      await sql.query`SELECT p.id, p.codigobarra, p.codigo as "codigoInterno", p.descripcion, p.descripcionale, p.empresa, s.iddeposito, d.codigo, SUM(s.existencia) as "existencia",p.pcostod, p.pcosto, p.preven, p.preven2, p.preven3, p.preven4, p.preven5, p.preven6, p.preven7, p.preven8, p.preven9, p.preven10, p.preven11, p.preven12, p.preven13, p.preven14, p.preven15, p.preven16, p.preven17, p.preven18, p.preven19, p.preven20
    FROM Producto p
    INNER JOIN STKDeposito s
    ON p.id = s.idproducto
    INNER JOIN deposito d
